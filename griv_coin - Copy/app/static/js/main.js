...
async function startQRScanner() {
    ...
    try {
        ...
        showNotification('Error accessing camera. Please make sure you have given camera permissions.', 'error');
    }
}
        ...
        if (video.readyState === video.HAVE_ENOUGH_DATA) {
            ...
            try {
                ...
                if (code) {
                    ...
                    const response = await fetch(`/qr/validate/${code.data}`);
                    if (response.ok) {
                        stopQRScanner();
                        window.location.href = `/qr/payment/${code.data}`;
                        return;
                    }
                }
            } catch (err) {
